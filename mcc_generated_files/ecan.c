...
#include <xc.h>
#include "ecan.h"
// Örne?in, ecan.c içinde global de?i?kenlerin tan?mlar?:
...
uint8_t CAN_receive(uCAN_MSG *tempCanMsg) {
    ...
    if (RXB0CONbits.RXFUL != 0) { //CheckRXB0
        ...
        // Ayn? i?lemler RXB1 için de tekrarlan?yor
...
void ECAN_Send_VehicleSpeed_Request(void) {
    ...
    ECAN_SetTxMsgId(0x7DF); // Örnek ID
...
void ECAN_SetTxMsgId(uint32_t id) {
    ...
    TXB0SIDH = (id >> 3) & 0xFF; // SIDH (üst 8 bit)
